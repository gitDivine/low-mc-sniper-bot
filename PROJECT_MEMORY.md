...
*   **Discovery Methodology (GeckoTerminal vs On-Chain):** Pool discovery uses GeckoTerminal's `/networks/solana/new_pools` endpoint instead of raw Raydium on-chain RPC logs. This provides valuable multi-DEX coverage (Pump.fun, Meteora, Raydium, Orca). However, it relies on a third-party proprietary indexer which introduces an unverified risk of silent filtering (e.g. dropping zero-liquidity or malformed pools). To mitigate data loss, pagination is maxed at `pages=10` (200 pools) per 5-minute poll. **MANDATORY FOLLOW-UP:** After Batch 2 completes, run a short, localized Raydium-v4 on-chain RPC signature scan to cross-check GT's Raydium pool counts for the same time window. If they match, GT is fully trusted; if GT is missing pools, discovery must be rewritten to use raw RPC.
